...
- keep live-runtime coverage around enemy-fighter patrol fallback when no hostile nest remains, so that no-target fallback patrol picks still consume only the `ants-runtime` stream while food regrowth and enemy production continue alongside them
- keep live-runtime coverage around the enemy-fighter near-hostile pressure-roll fallback branch, so fighters that still have a hostile nest available but are already inside pressure radius keep their patrol-versus-pressure choice isolated on `ants-runtime` during the normal gameplay update order
- keep live-runtime coverage around target-arrival fallback reselection so worker and fighter movement targets that complete during the normal gameplay update order still consume only the `ants-runtime` stream when they immediately pick a new wander or idle outcome
...
- keep live-runtime coverage around invalid carry-food recovery fallback so workers with stale carried-food references clear assignments and reselect behavior on `ants-runtime` without perturbing `food`, `enemy-economy`, `ants-spawn`, or `ants-effects`
