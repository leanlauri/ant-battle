...
- [x] Live runtime deterministic coverage now also exercises enemy-fighter patrol fallback when no hostile nest remains, confirming that no-target fallback patrol picks stay isolated on `ants-runtime` while food regrowth, enemy production, spawned reinforcements, and effect presentation continue in the normal gameplay update order
- [x] Live runtime deterministic coverage now also exercises the enemy-fighter near-hostile pressure-roll fallback branch, confirming that fighters already inside pressure radius still keep their patrol-versus-pressure choice isolated on `ants-runtime` while food regrowth, enemy production, spawned reinforcements, and effect presentation continue in the normal gameplay update order
- [x] Live runtime deterministic coverage now also exercises worker/fighter target-arrival fallback reselection, confirming movement-target completion paths stay isolated from `food`, `enemy-economy`, `ants-spawn`, and `ants-effects` changes during the gameplay update order while `ants-runtime` still owns their divergence
- [x] Live runtime deterministic coverage now also exercises invalidated worker food-target and assist-carry fallback reselection, and the runtime logic now keeps those worker fallback branches on the seeded `ants-runtime` stream instead of silently dropping to unseeded randomness
...
- [x] Spawned reinforcements now keep `ants-spawn` responsible only for placement and spawn-time variation, while each ant's later fallback decisions come from a per-ant `ants-runtime` substream, with live deterministic coverage that proves spawned post-placement behavior stays stable when only `ants-spawn` changes
- [x] Live runtime deterministic coverage now also exercises spawned worker focus-target routing and spawned enemy fighter near-hostile pressure decisions, confirming those first post-spawn branch choices stay isolated on `ants-runtime` while reinforcement placement remains isolated on `ants-spawn`
