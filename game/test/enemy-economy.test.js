...
const createLiveEnemyPatrolFallbackHarness = ({
  foodSeed,
  economySeed,
  spawnSeed,
  decisionSeed,
  ...
  const antSystem = new AntSystem({
    ...
    setupRandom: createSeededRandom('live-enemy-patrol-fallback-setup'),
    decisionSeed,
    decisionRandom: createSeededRandom(decisionSeed),
    effectRandom: createSeededRandom(effectSeed),
    spawnRandom: createSeededRandom(spawnSeed),
  ...
  const regrowthFood = foodSystem.items[0];
  foodSystem.pickUpFood(regrowthFood.id, 411, COLONY.player);
  foodSystem.dropFoodInNest(regrowthFood.id, 411, playerNest.id);

  playerNest.collapsed = true;
  playerNest.hp = 0;

  const enemyFighter = antSystem.ants.find((ant) => ant.faction === 'enemy' && ant.role === ANT_ROLE.fighter);

  enemyFighter.position.set(enemyNest.position.x + 6.5, enemyFighter.position.y, enemyNest.position.z - 1.5);
  enemyFighter.heading.set(0, 0, 1);
  enemyFighter.target.set(enemyFighter.position.x, 0, enemyFighter.position.z);
  enemyFighter.velocity.setScalar(0);
  enemyFighter.desiredVelocity.setScalar(0);
  enemyFighter.brainCooldown = 0;
...
const simulateLiveEnemyPatrolFallbackIntegration = ({
  foodSeed,
  economySeed,
  spawnSeed,
  decisionSeed,
  ...
  const harness = createLiveEnemyPatrolFallbackHarness({
    foodSeed,
    economySeed,
    spawnSeed,
    decisionSeed,
...
const createLiveEnemyPressureFallbackHarness = ({
  foodSeed,
  economySeed,
  spawnSeed,
  decisionSeed,
  ...
  const antSystem = new AntSystem({
    ...
    setupRandom: createSeededRandom('live-enemy-pressure-fallback-setup'),
    decisionSeed,
    decisionRandom: createSeededRandom(decisionSeed),
    effectRandom: createSeededRandom(effectSeed),
    spawnRandom: createSeededRandom(spawnSeed),
  ...
  const regrowthFood = foodSystem.items[0];
  foodSystem.pickUpFood(regrowthFood.id, 577, COLONY.player);
  foodSystem.dropFoodInNest(regrowthFood.id, 577, playerNest.id);

  const enemyFighter = antSystem.ants.find((ant) => ant.faction === 'enemy' && ant.role === ANT_ROLE.fighter);
  enemyFighter.position.set(playerNest.position.x + 8.5, enemyFighter.position.y, playerNest.position.z + 0.8);
  enemyFighter.heading.set(-1, 0, 0);
  enemyFighter.target.set(enemyFighter.position.x, 0, enemyFighter.position.z);
  enemyFighter.velocity.setScalar(0);
  enemyFighter.desiredVelocity.setScalar(0);
...
const simulateLiveEnemyPressureFallbackIntegration = ({
  foodSeed,
  economySeed,
  spawnSeed,
  decisionSeed,
  ...
  const harness = createLiveEnemyPressureFallbackHarness({
    foodSeed,
    economySeed,
    spawnSeed,
    decisionSeed,
  ...
  for (let step = 0; step < steps; step += 1) {
    ...
    pressureFallback ??= snapshotDecisionAnt(harness.enemyFighter);
...
const createLiveInvalidCarryRecoveryHarness = ({
  foodSeed,
  economySeed,
  spawnSeed,
  decisionSeed,
  ...
  const antSystem = new AntSystem({
    ...
    setupRandom: createSeededRandom('live-invalid-carry-recovery-setup'),
    decisionSeed,
    decisionRandom: createSeededRandom(decisionSeed),
    effectRandom: createSeededRandom(effectSeed),
    spawnRandom: createSeededRandom(spawnSeed),
  ...
  const regrowthFood = foodSystem.items[0];
  foodSystem.pickUpFood(regrowthFood.id, 811, COLONY.player);
  foodSystem.dropFoodInNest(regrowthFood.id, 811, playerNest.id);

  const carryWorker = antSystem.ants.find((ant) => ant.faction === 'player' && ant.role === ANT_ROLE.worker);
  carryWorker.position.set(-14, carryWorker.position.y, -12);
  carryWorker.heading.set(1, 0, 0);
  carryWorker.target.set(-10, 0, -12);
  carryWorker.velocity.setScalar(0);
  carryWorker.desiredVelocity.setScalar(0);
  carryWorker.action = 'carry-food';
  carryWorker.targetFoodId = 999_991;
  carryWorker.carryingFoodId = 999_991;
  carryWorker.queuedNestSlot = foodSystem.reserveNestSlot(carryWorker.id, carryWorker.position, carryWorker.homeNestId);
  carryWorker.nestApproachStage = 'entrance';
  carryWorker.brainCooldown = 0;
  carryWorker.logicCooldown = 0;
...
const simulateLiveInvalidCarryRecoveryIntegration = ({
  foodSeed,
  economySeed,
  spawnSeed,
  decisionSeed,
  ...
  const harness = createLiveInvalidCarryRecoveryHarness({
    foodSeed,
    economySeed,
    spawnSeed,
    decisionSeed,
  ...
  for (let step = 0; step < steps; step += 1) {
    ...
    if (recoverySummary == null) {
      recoverySummary = {
        ...snapshotDecisionAnt(harness.carryWorker),
        carryingFoodId: harness.carryWorker.carryingFoodId,
        queuedNestSlot: harness.carryWorker.queuedNestSlot != null,
        queueAssignmentExists: harness.foodSystem.queueAssignments.has(harness.carryWorker.id),
      };
    }
...
const createLiveSpawnedFallbackHarness = ({
  foodSeed,
  economySeed,
  spawnSeed,
  decisionSeed,
  ...
  const antSystem = new AntSystem({
    ...
    setupRandom: createSeededRandom('live-spawned-fallback-setup'),
    decisionSeed,
    decisionRandom: createSeededRandom(decisionSeed),
    effectRandom: createSeededRandom(effectSeed),
    spawnRandom: createSeededRandom(spawnSeed),
  ...
  const regrowthFood = foodSystem.items[0];
  foodSystem.pickUpFood(regrowthFood.id, 515, COLONY.player);
  foodSystem.dropFoodInNest(regrowthFood.id, 515, playerNest.id);
...
const simulateLiveSpawnedFallbackIntegration = ({
  foodSeed,
  economySeed,
  spawnSeed,
  decisionSeed,
  ...
  const harness = createLiveSpawnedFallbackHarness({
    foodSeed,
    economySeed,
    spawnSeed,
    decisionSeed,
  ...
  const timeline = [];
  let previousAntCount = harness.antSystem.ants.length;
  let spawnedDecisionAnt = null;
  let spawnedDecisionSummary = null;
  ...
  for (let step = 0; step < steps; step += 1) {
    ...
    if (harness.antSystem.ants.length > previousAntCount) {
      ...
      if (spawnedDecisionAnt == null && latestAnts.length) {
        spawnedDecisionAnt = latestAnts[0];
        for (const ant of latestAnts.slice(1)) {
        ...
        spawnedDecisionAnt.position.set(-6, spawnedDecisionAnt.position.y, -5);
        spawnedDecisionAnt.heading.set(1, 0, 0);
        spawnedDecisionAnt.target.set(spawnedDecisionAnt.position.x, 0, spawnedDecisionAnt.position.z);
        spawnedDecisionAnt.velocity.setScalar(0);
        spawnedDecisionAnt.desiredVelocity.setScalar(0);
        spawnedDecisionAnt.brainCooldown = 0;
        spawnedDecisionAnt.logicCooldown = 999;
        continue;
      }
    }

    if (spawnedDecisionAnt && spawnedDecisionSummary == null) {
      spawnedDecisionSummary = snapshotDecisionAnt(spawnedDecisionAnt);
      break;
    }
  }
  ...
  return {
    ...
    spawnedAnts: snapshotEnemySpawnedAnts(harness.antSystem, harness.enemyNest.id),
    regrownFood: simplifyFood(harness.regrowthFood),
    spawnedDecisionAnt: spawnedDecisionSummary,
    effects: snapshotEffectState(harness.antSystem),
  };
};

const createLiveSpawnedFocusPressureHarness = ({
  foodSeed,
  economySeed,
  spawnSeed,
  decisionSeed,
  ...
  const antSystem = new AntSystem({
    ...
    setupRandom: createSeededRandom('live-spawned-focus-pressure-setup'),
    decisionSeed,
    decisionRandom: createSeededRandom(decisionSeed),
    effectRandom: createSeededRandom(effectSeed),
    spawnRandom: createSeededRandom(spawnSeed),
  ...
  const regrowthFood = foodSystem.items[0];
  foodSystem.pickUpFood(regrowthFood.id, 711, COLONY.player);
  foodSystem.dropFoodInNest(regrowthFood.id, 711, playerNest.id);

  antSystem.spawnAntBatch({ nestId: playerNest.id, role: ANT_ROLE.worker, count: 1 });
  antSystem.spawnAntBatch({ nestId: enemyNest.id, role: ANT_ROLE.fighter, count: 2 });

  const spawnedPlayerWorker = antSystem.ants.find((ant) => ant.homeNestId === playerNest.id && ant.role === ANT_ROLE.worker);
  const spawnedEnemyFighters = antSystem.ants.filter((ant) => ant.homeNestId === enemyNest.id && ant.role === ANT_ROLE.fighter);
  const spawnedEnemyPressureFighter = spawnedEnemyFighters[0];

  spawnedPlayerWorker.position.set(-22, spawnedPlayerWorker.position.y, -20);
  spawnedPlayerWorker.heading.set(1, 0, 0);
  spawnedPlayerWorker.target.set(spawnedPlayerWorker.position.x, 0, spawnedPlayerWorker.position.z);
  spawnedPlayerWorker.velocity.setScalar(0);
  spawnedPlayerWorker.desiredVelocity.setScalar(0);
  spawnedPlayerWorker.brainCooldown = 0;
  spawnedPlayerWorker.logicCooldown = 999;

  spawnedEnemyPressureFighter.position.set(9.5, spawnedEnemyPressureFighter.position.y, 0.8);
  spawnedEnemyPressureFighter.heading.set(-1, 0, 0);
  spawnedEnemyPressureFighter.target.set(spawnedEnemyPressureFighter.position.x, 0, spawnedEnemyPressureFighter.position.z);
  spawnedEnemyPressureFighter.velocity.setScalar(0);
  spawnedEnemyPressureFighter.desiredVelocity.setScalar(0);
  spawnedEnemyPressureFighter.brainCooldown = 0;
  spawnedEnemyPressureFighter.logicCooldown = 999;

  for (const fillerAnt of spawnedEnemyFighters.slice(1)) {
    fillerAnt.brainCooldown = 999;
    fillerAnt.logicCooldown = 999;
    fillerAnt.velocity.setScalar(0);
    fillerAnt.desiredVelocity.setScalar(0);
    fillerAnt.target.set(fillerAnt.position.x, 0, fillerAnt.position.z);
  }
...
const simulateLiveSpawnedFocusPressureIntegration = ({
  foodSeed,
  economySeed,
  spawnSeed,
  decisionSeed,
  ...
  const harness = createLiveSpawnedFocusPressureHarness({
    foodSeed,
    economySeed,
    spawnSeed,
    decisionSeed,
  ...
  const timeline = [];
  let previousAntCount = harness.antSystem.ants.length;
  let spawnedFocusSummary = null;
  let spawnedPressureSummary = null;
  ...
  for (let step = 0; step < steps; step += 1) {
    ...
    spawnedFocusSummary ??= snapshotDecisionAnt(harness.spawnedPlayerWorker);
    spawnedPressureSummary ??= snapshotDecisionAnt(harness.spawnedEnemyPressureFighter);
...
describe('enemy economy seeded runtime paths', () => {
  ...
  test('keeps live enemy fighter no-hostile patrol fallback isolated from unrelated seeded runtime streams', () => {
    const foodSeed = deriveSeed('ant-battle-level-12', 'food');
    const economySeed = deriveSeed('ant-battle-level-12', 'enemy-economy');
    const spawnSeed = deriveSeed('ant-battle-level-12', 'ants-spawn');
    const decisionSeed = deriveSeed('ant-battle-level-12', 'ants-runtime');
    ...
    const baseline = simulateLiveEnemyPatrolFallbackIntegration({
      foodSeed,
      economySeed,
      spawnSeed,
      decisionSeed,
    ...
    const repeat = simulateLiveEnemyPatrolFallbackIntegration({
      foodSeed,
      economySeed,
      spawnSeed,
      decisionSeed,
    ...
    const runtimeVariant = simulateLiveEnemyPatrolFallbackIntegration({
      foodSeed,
      economySeed,
      spawnSeed,
      decisionSeed: deriveSeed('ant-battle-level-13', 'ants-runtime'),
    ...
    const foodVariant = simulateLiveEnemyPatrolFallbackIntegration({
      foodSeed: deriveSeed('ant-battle-level-13', 'food'),
      economySeed,
      spawnSeed,
      decisionSeed,
    ...
    const economyVariant = simulateLiveEnemyPatrolFallbackIntegration({
      foodSeed,
      economySeed: deriveSeed('ant-battle-level-13', 'enemy-economy'),
      spawnSeed,
      decisionSeed,
    ...
    const spawnVariant = simulateLiveEnemyPatrolFallbackIntegration({
      foodSeed,
      economySeed,
      spawnSeed: deriveSeed('ant-battle-level-13', 'ants-spawn'),
      decisionSeed,
    ...
    const effectsVariant = simulateLiveEnemyPatrolFallbackIntegration({
      foodSeed,
      economySeed,
      spawnSeed,
      decisionSeed,
    ...
    expect(baseline).toEqual(repeat);
    expect(baseline.enemyFighter.action).toBe('patrol');
  ...
  test('keeps live enemy fighter near-hostile pressure fallback isolated from unrelated seeded runtime streams', () => {
    const foodSeed = deriveSeed('ant-battle-level-12', 'food');
    const economySeed = deriveSeed('ant-battle-level-12', 'enemy-economy');
    const spawnSeed = deriveSeed('ant-battle-level-12', 'ants-spawn');
    const decisionSeed = deriveSeed('ant-battle-level-12', 'ants-runtime');
    ...
    const baseline = simulateLiveEnemyPressureFallbackIntegration({
      foodSeed,
      economySeed,
      spawnSeed,
      decisionSeed,
    ...
    const repeat = simulateLiveEnemyPressureFallbackIntegration({
      foodSeed,
      economySeed,
      spawnSeed,
      decisionSeed,
    ...
    const runtimeVariant = simulateLiveEnemyPressureFallbackIntegration({
      foodSeed,
      economySeed,
      spawnSeed,
      decisionSeed: deriveSeed('ant-battle-level-13', 'ants-runtime'),
    ...
    const foodVariant = simulateLiveEnemyPressureFallbackIntegration({
      foodSeed: deriveSeed('ant-battle-level-13', 'food'),
      economySeed,
      spawnSeed,
      decisionSeed,
    ...
    const economyVariant = simulateLiveEnemyPressureFallbackIntegration({
      foodSeed,
      economySeed: deriveSeed('ant-battle-level-13', 'enemy-economy'),
      spawnSeed,
      decisionSeed,
    ...
    const spawnVariant = simulateLiveEnemyPressureFallbackIntegration({
      foodSeed,
      economySeed,
      spawnSeed: deriveSeed('ant-battle-level-13', 'ants-spawn'),
      decisionSeed,
    ...
    const effectsVariant = simulateLiveEnemyPressureFallbackIntegration({
      foodSeed,
      economySeed,
      spawnSeed,
      decisionSeed,
    ...
    expect(baseline).toEqual(repeat);
    expect(baseline.pressureFallback.action).toBe('patrol');
    expect(runtimeVariant.pressureFallback.action).toBe('focus-target');

    expect(baseline.pressureFallback).not.toEqual(runtimeVariant.pressureFallback);
    expect(baseline.timeline).toEqual(runtimeVariant.timeline);
    expect(baseline.regrownFood).toEqual(runtimeVariant.regrownFood);
    expect(baseline.effects).toEqual(runtimeVariant.effects);

    expect(baseline.pressureFallback).toEqual(foodVariant.pressureFallback);
    expect(baseline.timeline).toEqual(foodVariant.timeline);
    expect(baseline.spawnedAnts).toEqual(foodVariant.spawnedAnts);
    expect(baseline.regrownFood).not.toEqual(foodVariant.regrownFood);

    expect(baseline.pressureFallback).toEqual(economyVariant.pressureFallback);
    expect(baseline.regrownFood).toEqual(economyVariant.regrownFood);
    expect(baseline.timeline).not.toEqual(economyVariant.timeline);

    expect(baseline.pressureFallback).toEqual(spawnVariant.pressureFallback);
    expect(baseline.timeline).toEqual(spawnVariant.timeline);
    expect(baseline.regrownFood).toEqual(spawnVariant.regrownFood);
    expect(baseline.spawnedAnts).not.toEqual(spawnVariant.spawnedAnts);

    expect(baseline.pressureFallback).toEqual(effectsVariant.pressureFallback);
    expect(baseline.timeline).toEqual(effectsVariant.timeline);
    expect(baseline.regrownFood).toEqual(effectsVariant.regrownFood);
    expect(baseline.spawnedAnts).toEqual(effectsVariant.spawnedAnts);
    expect(baseline.effects).toEqual(effectsVariant.effects);
  ...
  test('keeps invalid carry-food recovery fallback isolated from unrelated seeded runtime streams', () => {
    const foodSeed = deriveSeed('ant-battle-level-12', 'food');
    const economySeed = deriveSeed('ant-battle-level-12', 'enemy-economy');
    const spawnSeed = deriveSeed('ant-battle-level-12', 'ants-spawn');
    const decisionSeed = deriveSeed('ant-battle-level-12', 'ants-runtime');
    ...
    const baseline = simulateLiveInvalidCarryRecoveryIntegration({
      foodSeed,
      economySeed,
      spawnSeed,
      decisionSeed,
    ...
    const repeat = simulateLiveInvalidCarryRecoveryIntegration({
      foodSeed,
      economySeed,
      spawnSeed,
      decisionSeed,
    ...
    const runtimeVariant = simulateLiveInvalidCarryRecoveryIntegration({
      foodSeed,
      economySeed,
      spawnSeed,
      decisionSeed: deriveSeed('ant-battle-level-13', 'ants-runtime'),
    ...
    const foodVariant = simulateLiveInvalidCarryRecoveryIntegration({
      foodSeed: deriveSeed('ant-battle-level-13', 'food'),
      economySeed,
      spawnSeed,
      decisionSeed,
    ...
    const economyVariant = simulateLiveInvalidCarryRecoveryIntegration({
      foodSeed,
      economySeed: deriveSeed('ant-battle-level-13', 'enemy-economy'),
      spawnSeed,
      decisionSeed,
    ...
    const spawnVariant = simulateLiveInvalidCarryRecoveryIntegration({
      foodSeed,
      economySeed,
      spawnSeed: deriveSeed('ant-battle-level-13', 'ants-spawn'),
      decisionSeed,
    ...
    const effectsVariant = simulateLiveInvalidCarryRecoveryIntegration({
      foodSeed,
      economySeed,
      spawnSeed,
      decisionSeed,
    ...
    expect(baseline).toEqual(repeat);
    expect(baseline.recoverySummary.carryingFoodId).toBeNull();
    expect(baseline.recoverySummary.queuedNestSlot).toBe(false);
    expect(baseline.recoverySummary.queueAssignmentExists).toBe(false);
    expect(baseline.recoverySummary.action).not.toBe('carry-food');

    expect(baseline.recoverySummary).not.toEqual(runtimeVariant.recoverySummary);
    expect(baseline.timeline).toEqual(runtimeVariant.timeline);
    expect(baseline.regrownFood).toEqual(runtimeVariant.regrownFood);
    expect(baseline.effects).toEqual(runtimeVariant.effects);

    expect(baseline.recoverySummary).toEqual(foodVariant.recoverySummary);
    expect(baseline.timeline).toEqual(foodVariant.timeline);
    expect(baseline.spawnedAnts).toEqual(foodVariant.spawnedAnts);
    expect(baseline.regrownFood).not.toEqual(foodVariant.regrownFood);

    expect(baseline.recoverySummary).toEqual(economyVariant.recoverySummary);
    expect(baseline.regrownFood).toEqual(economyVariant.regrownFood);
    expect(baseline.timeline).not.toEqual(economyVariant.timeline);

    expect(baseline.recoverySummary).toEqual(spawnVariant.recoverySummary);
    expect(baseline.timeline).toEqual(spawnVariant.timeline);
    expect(baseline.regrownFood).toEqual(spawnVariant.regrownFood);
    expect(baseline.spawnedAnts).not.toEqual(spawnVariant.spawnedAnts);

    expect(baseline.recoverySummary).toEqual(effectsVariant.recoverySummary);
    expect(baseline.timeline).toEqual(effectsVariant.timeline);
    expect(baseline.regrownFood).toEqual(effectsVariant.regrownFood);
    expect(baseline.spawnedAnts).toEqual(effectsVariant.spawnedAnts);
    expect(baseline.effects).toEqual(effectsVariant.effects);
  ...
  test('keeps spawned reinforcement fallback decisions on ants-runtime after placement uses ants-spawn', () => {
    const foodSeed = deriveSeed('ant-battle-level-12', 'food');
    const economySeed = deriveSeed('ant-battle-level-12', 'enemy-economy');
    const spawnSeed = deriveSeed('ant-battle-level-12', 'ants-spawn');
    const decisionSeed = deriveSeed('ant-battle-level-12', 'ants-runtime');
    ...
    const baseline = simulateLiveSpawnedFallbackIntegration({
      foodSeed,
      economySeed,
      spawnSeed,
      decisionSeed,
    ...
    const repeat = simulateLiveSpawnedFallbackIntegration({
      foodSeed,
      economySeed,
      spawnSeed,
      decisionSeed,
    ...
    const runtimeVariant = simulateLiveSpawnedFallbackIntegration({
      foodSeed,
      economySeed,
      spawnSeed,
      decisionSeed: deriveSeed('ant-battle-level-13', 'ants-runtime'),
    ...
    const foodVariant = simulateLiveSpawnedFallbackIntegration({
      foodSeed: deriveSeed('ant-battle-level-13', 'food'),
      economySeed,
      spawnSeed,
      decisionSeed,
    ...
    const economyVariant = simulateLiveSpawnedFallbackIntegration({
      foodSeed,
      economySeed: deriveSeed('ant-battle-level-13', 'enemy-economy'),
      spawnSeed,
      decisionSeed,
    ...
    const spawnVariant = simulateLiveSpawnedFallbackIntegration({
      foodSeed,
      economySeed,
      spawnSeed: deriveSeed('ant-battle-level-13', 'ants-spawn'),
      decisionSeed,
    ...
    const effectsVariant = simulateLiveSpawnedFallbackIntegration({
      foodSeed,
      economySeed,
      spawnSeed,
      decisionSeed,
    ...
    expect(baseline).toEqual(repeat);
    expect(baseline.spawnedDecisionAnt).not.toEqual(runtimeVariant.spawnedDecisionAnt);
    expect(baseline.timeline).toEqual(runtimeVariant.timeline);
    expect(baseline.regrownFood).toEqual(runtimeVariant.regrownFood);
    expect(baseline.effects).toEqual(runtimeVariant.effects);

    expect(baseline.spawnedDecisionAnt).toEqual(foodVariant.spawnedDecisionAnt);
    expect(baseline.timeline).toEqual(foodVariant.timeline);
    expect(baseline.spawnedAnts).toEqual(foodVariant.spawnedAnts);
    expect(baseline.regrownFood).not.toEqual(foodVariant.regrownFood);

    expect(baseline.spawnedDecisionAnt).toEqual(economyVariant.spawnedDecisionAnt);
    expect(baseline.timeline).not.toEqual(economyVariant.timeline);

    expect(baseline.spawnedDecisionAnt).toEqual(spawnVariant.spawnedDecisionAnt);
    expect(baseline.timeline).toEqual(spawnVariant.timeline);
    expect(baseline.regrownFood).toEqual(spawnVariant.regrownFood);
    expect(baseline.spawnedAnts).not.toEqual(spawnVariant.spawnedAnts);

    expect(baseline.spawnedDecisionAnt).toEqual(effectsVariant.spawnedDecisionAnt);
    expect(baseline.timeline).toEqual(effectsVariant.timeline);
    expect(baseline.regrownFood).toEqual(effectsVariant.regrownFood);
    expect(baseline.spawnedAnts).toEqual(effectsVariant.spawnedAnts);
    expect(baseline.effects).toEqual(effectsVariant.effects);
  ...
  test('keeps spawned worker focus-target routing and spawned enemy fighter pressure decisions isolated from unrelated seeded runtime streams', () => {
    const foodSeed = deriveSeed('ant-battle-level-12', 'food');
    const economySeed = deriveSeed('ant-battle-level-12', 'enemy-economy');
    const spawnSeed = deriveSeed('ant-battle-level-12', 'ants-spawn');
    const decisionSeed = deriveSeed('ant-battle-level-12', 'ants-runtime');
    ...
    const baseline = simulateLiveSpawnedFocusPressureIntegration({
      foodSeed,
      economySeed,
      spawnSeed,
      decisionSeed,
    ...
    const repeat = simulateLiveSpawnedFocusPressureIntegration({
      foodSeed,
      economySeed,
      spawnSeed,
      decisionSeed,
    ...
    const runtimeVariant = simulateLiveSpawnedFocusPressureIntegration({
      foodSeed,
      economySeed,
      spawnSeed,
      decisionSeed: deriveSeed('ant-battle-level-13', 'ants-runtime'),
    ...
    const foodVariant = simulateLiveSpawnedFocusPressureIntegration({
      foodSeed: deriveSeed('ant-battle-level-13', 'food'),
      economySeed,
      spawnSeed,
      decisionSeed,
    ...
    const economyVariant = simulateLiveSpawnedFocusPressureIntegration({
      foodSeed,
      economySeed: deriveSeed('ant-battle-level-13', 'enemy-economy'),
      spawnSeed,
      decisionSeed,
    ...
    const spawnVariant = simulateLiveSpawnedFocusPressureIntegration({
      foodSeed,
      economySeed,
      spawnSeed: deriveSeed('ant-battle-level-13', 'ants-spawn'),
      decisionSeed,
    ...
    const effectsVariant = simulateLiveSpawnedFocusPressureIntegration({
      foodSeed,
      economySeed,
      spawnSeed,
      decisionSeed,
    ...
    expect(baseline).toEqual(repeat);
    expect(baseline.spawnedFocusSummary).not.toEqual(runtimeVariant.spawnedFocusSummary);
    expect(baseline.spawnedPressureSummary).not.toEqual(runtimeVariant.spawnedPressureSummary);
    expect(baseline.timeline).toEqual(runtimeVariant.timeline);
    expect(baseline.regrownFood).toEqual(runtimeVariant.regrownFood);
    expect(baseline.effects).toEqual(runtimeVariant.effects);

    expect(baseline.spawnedFocusSummary).toEqual(foodVariant.spawnedFocusSummary);
    expect(baseline.spawnedPressureSummary).toEqual(foodVariant.spawnedPressureSummary);
    expect(baseline.timeline).toEqual(foodVariant.timeline);
    expect(baseline.spawnedAnts).toEqual(foodVariant.spawnedAnts);
    expect(baseline.regrownFood).not.toEqual(foodVariant.regrownFood);

    expect(baseline.spawnedFocusSummary).toEqual(economyVariant.spawnedFocusSummary);
    expect(baseline.spawnedPressureSummary).toEqual(economyVariant.spawnedPressureSummary);
    expect(baseline.regrownFood).toEqual(economyVariant.regrownFood);
    expect(baseline.timeline).not.toEqual(economyVariant.timeline);

    expect(baseline.spawnedFocusSummary).toEqual(spawnVariant.spawnedFocusSummary);
    expect(baseline.spawnedPressureSummary).toEqual(spawnVariant.spawnedPressureSummary);
    expect(baseline.timeline).toEqual(spawnVariant.timeline);
    expect(baseline.regrownFood).toEqual(spawnVariant.regrownFood);
    expect(baseline.spawnedAnts).not.toEqual(spawnVariant.spawnedAnts);

    expect(baseline.spawnedFocusSummary).toEqual(effectsVariant.spawnedFocusSummary);
    expect(baseline.spawnedPressureSummary).toEqual(effectsVariant.spawnedPressureSummary);
    expect(baseline.timeline).toEqual(effectsVariant.timeline);
    expect(baseline.spawnedAnts).toEqual(effectsVariant.spawnedAnts);
    expect(baseline.regrownFood).toEqual(effectsVariant.regrownFood);
    expect(baseline.effects).toEqual(effectsVariant.effects);
