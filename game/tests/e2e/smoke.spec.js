...
test('boots through title, level select, gameplay, and victory progression flow', async ({ page }) => {
  ...
  expect(battlefieldCameraState.frustum.bottom).toBeLessThan(battlefieldCameraState.frustum.top * -1);
  ...
  expect(clampedBattlefieldState.target.x).toBeLessThan(46);
  expect(clampedBattlefieldState.target.z).toBeLessThan(46);
